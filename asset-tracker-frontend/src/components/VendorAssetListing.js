...
import { getVendorsAsset } from "../api/apiCalls";
...
const VendorAssetListing = () => {
  ...
  const loadData = async (pageIndex, size = 5) => {
    ...
    try {
      const response = await getVendorsAsset(
        auth.companyId,
        params.vendorId,
        searchData,
        pageIndex,
        ...
        <div className="container">
          ...
          <div className="card mt-3">
            <div className="card-header text-center">
              <h3>
                {assetPages.page && content.length !== 0
                  ? `Assets supplied by ${content[0].vendor[0].vendorName}`
                  : `Assets`}{" "}
...
export default VendorAssetListing;
